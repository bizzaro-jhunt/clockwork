...
Password for the authenticating user.  Normally, B<cw-mesh> will prompt for
this, so you don't need (or want) to specify it on the command line.  Some
precautions are taken to mitigate the exposure of the password to tools like
B<ps>(1), but the most secure option is to not use this one.

=item B<--mastercert> I</path/to/cert.pub>
