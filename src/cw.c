...
#define STRL_INIT_LEN   16
#define STRL_EXP_FACTOR 8
#define STRL_EXPAND(x) (x / STRL_EXP_FACTOR + 1) * STRL_EXP_FACTOR

static int s_strl_expand(cw_strl_t *sl, size_t expand)
{
	assert(sl);         // LCOV_EXCL_LINE
	assert(expand > 0); // LCOV_EXCL_LINE

	char **s;
	expand = STRL_EXPAND(expand) + sl->len;
...
static int s_strl_reduce(cw_strl_t *sl)
{
...
static size_t s_strl_capacity(cw_strl_t *sl)
{
...
/**
  ...
  be freed via @cw_strl_free.

  On failure, any memory allocated by `cw_strl_new` will be
...
cw_strl_t* cw_strl_new(char **src)
{
	...
	if (src) {
		...
		while (*t++)
			;
		sl->num = t - src - 1;
		sl->len = STRL_EXPAND(sl->num);
	} else {
		sl->num = 0;
		sl->len = STRL_INIT_LEN;
	}
...
/**
  ...
  <code>
  cw_strl_t *orig = generate_a_list_of_strings()

  // this statement...
  cw_strl_t *new1 = cw_strl_dup(orig);

  // ...is equivalent to this one
  cw_strl_t *new2 = cw_strl_new(orig->strings);
  </code>
...
{
	return cw_strl_new(orig->strings);
}

/**
  Free the $sl string list.
 */
void cw_strl_free(cw_strl_t *sl)
{
	size_t i;
	if (sl) {
...
/**
  ...
  In order to do that, $sl will first be sorted (using @cw_strl_sort
  and `STRL_ASC`), so that duplicate values are adjacent.

  Example:

  <code>
  cw_strl_t *fruit = cw_strl_new(NULL);

  cw_strl_add(fruit, "pear");
  cw_strl_add(fruit, "banana");
  cw_strl_add(fruit, "pear");
  cw_strl_add(fruit, "apple");
  cw_strl_add(fruit, "pear");
 ...
 */
void cw_strl_uniq(cw_strl_t *sl)
{
	assert(sl); // LCOV_EXCL_LINE
 ...
 */
int cw_strl_search(const cw_strl_t *sl, const char* needle)
{
	assert(sl);     // LCOV_EXCL_LINE
 ...
 */
int cw_strl_add(cw_strl_t *sl, const char* str)
{
	assert(sl);  // LCOV_EXCL_LINE
	assert(str); // LCOV_EXCL_LINE

	/* expand as needed */
	if (s_strl_capacity(sl) == 0 && s_strl_expand(sl, 1) != 0) {
		return -1;
	}
...
int cw_strl_add_all(cw_strl_t *dst, const cw_strl_t *src)
{
	...
	if (s_strl_capacity(dst) < src->num && s_strl_expand(dst, src->num)) {
		return -1;
	}
...
/**
  ...
  <code>
  while (cw_strl_remove(sl, "a string") == 0)
      ;
  </code>
  ...
  Otherwise, returns non-zero.
 */
int cw_strl_remove(cw_strl_t *sl, const char *str)
{
	assert(sl);  // LCOV_EXCL_LINE
	assert(str); // LCOV_EXCL_LINE
...
{
	cw_strl_t *intersect = cw_strl_new(NULL);
	size_t i, j;

	for_each_string(a,i) {
		for_each_string(b,j) {
			if (strcmp(a->strings[i], b->strings[j]) == 0) {
				cw_strl_add(intersect, a->strings[i]);
			}
		}
	}
...
int cw_strl_diff(cw_strl_t *a, cw_strl_t *b)
{
	...
	for_each_string(a,i) {
		if (cw_strl_search(b, a->strings[i]) != 0
		 || cw_strl_search(a, b->strings[i]) != 0) {
			return 0;
		}
	}
...
/**
  ...
  Examples:
  <code>
  cw_strl_t *l = cw_strl_new(NULL);
  cw_strl_add(l, "red");
  cw_strl_add(l, "green");
  cw_strl_add(l, "blue");

  // A few different ways to join stringlists:
  cw_strl_join(l, ", ");    // "red, green, blue"
  cw_strl_join(l, " and "); // "red and green and blue"
  cw_strl_join(l, ":");     // "red:green:blue"
  </code>

  For the special case of an empty list, `cw_strl_join` will return
...
/**
  ...
  <code>
  cw_strl_t *l = cw_strl_split("one::two::three", 15, "::", 0);
