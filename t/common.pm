...
sub disassemble_ok
{
	...
	my $pid = fork;
	$T->BAIL_OUT("Failed to fork for pendulum disassembler run: $!") if $pid < 0;
	if ($pid == 0) {
		open STDOUT, ">&", $stdout
			or die "Failed to reopen stdout: $!\n";
		open STDERR, ">&", $stderr
			or die "Failed to reopen stderr: $!\n";
		exec $PN, "-d", $target or die "Failed to exec $PN: $!\n";
	}
	...
	if ($@) {
		if ($@ =~ m/^timed out/) {
			kill 9, $pid;
			$T->ok(0, "$message: timed out disassembling pendulum script");
			return 0;
		}
		$T->ok(0, "$message: exception raised - $@");
		return 0;
