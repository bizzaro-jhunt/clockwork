...
static void _ctap_eq_u64(uint64_t x, uint64_t y, const char *msg, const char *file, unsigned long line)
{
	if (!_ctap_assert(x == y, msg, 1, file, line)) {
		_ctap_diag(stderr, "         got: %lu", x);
		_ctap_diag(stderr, "    expected: %lu", y);
	}
}
...
{
	if (!_ctap_assert(x == y, msg, 1, file, line)) {
		_ctap_diag(stderr, "         got: %li", x);
		_ctap_diag(stderr, "    expected: %li", y);
	}
}
...
static int _ctap_junkvar = 0;
#define subtest for (_ctap_junkvar = 0; _ctap_junkvar < 1; _ctap_junkvar++)
